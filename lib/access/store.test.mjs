...
const admin = { id: 'test-admin', role: 'ADMIN', projectScopes:{[project]:'COMPANY'}, memberships: [] };
...
beforeEach(async () => {
  ...
  for (const slug of [project, personal]) {
    await prisma.flowProject.create({ data: { slug, title: slug, scope:slug===personal?'PERSONAL':'COMPANY',personalGroup:slug===personal?'PORTFOLIO':null, position: 999, categories: { create: {
...
test('account passwords allow fewer than 12 characters but still reject empty or oversized values',async()=>{
  const body={action:'createUser',username:'short-password',name:'Short Password',role:'MEMBER'};
  await manageAccess(owner,{...body,password:'x'});
  assert.equal((await resolveActor(await loginAccount(body.username,'x'))).role,'MEMBER');
  await assert.rejects(()=>manageAccess(owner,{...body,username:'empty-password',password:''}),status(400));
  await assert.rejects(()=>manageAccess(owner,{...body,username:'long-password',password:'x'.repeat(129)}),status(400));
});
...
test('administrators cannot change themselves, owners or admin roles; members cannot manage access', async () => {
  ...
  const update = { action: 'updateUser', active: true, role: 'ADMIN', projectScopes:{[project]:'COMPANY'}, memberships: [] };
...
test('임의 ID의 신규 프로젝트도 DB 소속을 따라 기존 세션의 권한에 즉시 반영된다', async () => {
  const privateSlug = 'test-dynamic-private';
  const companySlug = 'personal-looking-company';
  try {
    await prisma.flowProject.createMany({data:[
      {slug:privateSlug,title:'동적 개인',scope:'PERSONAL',personalGroup:'TOY',position:999},
      {slug:companySlug,title:'동적 회사',scope:'COMPANY',position:999},
    ]});
    await user(admin);
    const token=await issueSession(admin.id);
    let actor=await resolveActor(token);
    assert.equal(canProject(actor,privateSlug,'read'),false);
    assert.equal(canProject(actor,companySlug,'write'),true);
    assert.equal(actor.projectScopes[privateSlug],undefined);
    await prisma.accessMembership.create({data:{userId:admin.id,projectSlug:privateSlug,role:'VIEWER'}});
    actor=await resolveActor(token);
    assert.equal(actor.projectScopes[privateSlug],'PERSONAL');
    assert.equal(canProject(actor,privateSlug,'read'),true);
    assert.equal(canProject(actor,privateSlug,'write'),false);
    await prisma.accessMembership.deleteMany({where:{userId:admin.id,projectSlug:privateSlug}});
    assert.equal(canProject(await resolveActor(token),privateSlug,'read'),false);
  } finally {
    await prisma.accessMembership.deleteMany({where:{projectSlug:{in:[privateSlug,companySlug]}}});
    await prisma.flowProject.deleteMany({where:{slug:{in:[privateSlug,companySlug]}}});
  }
});
