...
test("copy includes only work linked to registered company projects", () => {
  const report = {
    version: 1,
    revision: 1,
    ...
    evidence: {
      ...evidence,
      projects: [...evidence.projects, { key: "app:common", title: "공통" }, { key: "app:personal-project", title: "개인 프로젝트" }, { key: "app:__personal_issues__", title: "개인 이슈" }],
    },
    summary: {
      items: [
        ...draft.items,
        { projectKey: "app:common", title: "일정 협의", sourceIds: ["c:2"] },
        { projectKey: "app:personal-project", title: "개인 프로젝트 작업", sourceIds: ["c:4"] },
  ...
  assert.equal(
    formatWorkSummary(report, new Set(["app:alpha"])),
    "09/14 작업내용\n`Alpha`\n- 로그인 권한 검증 개선",
  );
  assert.equal(
    formatWorkSummary({ ...report, summary: { items: [], excluded: [] } }, new Set(["app:alpha"])),
    "",
  );
  assert.equal(
    formatWorkSummary({ ...report, summary: { items: [report.summary.items[3]], excluded: [] } }, new Set(["app:alpha"])),
    "",
  );
  assert.equal(
    formatWorkSummary(report, new Set()),
    "",
  );
});
