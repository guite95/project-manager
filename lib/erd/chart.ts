import type snapshotType from "./tns-schema.json";
import type { FlowChart } from "../../components/flow/types";
export type ErdSnapshot = typeof snapshotType;
export type ErdModel = ErdSnapshot["models"][number];
...
export function fieldKeys(snapshot: ErdSnapshot, model: ErdModel, field: ErdField): string[] {
  return [
...
export function makeErdChart(snapshot: ErdSnapshot, domain: string, focus?: string): FlowChart {
  ...
  return {
    ...
    nodes: snapshot.models.filter(m => visible.has(m.name)).map(model => ({
      ...
      data: {
        ...
        entity: {
          ...
          fields: model.fields.filter(field => fieldKeys(snapshot, model, field).some(key => key === 'PK' || key === 'FK')).map(field => ({
            name: field.column, type: field.type, optional: field.optional, keys: fieldKeys(snapshot, model, field),
          })),
        },
      },
    })),
    ...
    howToRead: [
      ...
      '참조 배지가 있는 테이블은 다른 영역의 연결 대상입니다. 영역 화면은 해당 영역에서 나가는 모든 FK를 포함합니다. 테이블 선택은 현재 배치에서 직접 연결된 관계를 강조하며, 선택 테이블 연결만 보기에서는 들어오는 FK도 함께 표시합니다.',
