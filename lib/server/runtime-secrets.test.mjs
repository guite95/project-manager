...
test('migration reader accepts only its own DB file and Prisma cannot fall back to app runtime secrets', async t => {
  const create = await reader(), dir = fixture(t);
  rmSync(join(dir, 'g-aaaaaa/SESSION_SECRET'));
  const read = create({ directory: dir, ownerUid: process.getuid(), profile: 'migration' });
  assert.equal(read('DATABASE_URL'), 'postgresql://fixture:a@db/project_management');
  assert.throws(() => read('SESSION_SECRET'), /RUNTIME_SECRET_UNAVAILABLE/);
  const { getMigrationDatabaseUrl } = await import('./runtime-secrets.mjs');
  assert.equal(typeof getMigrationDatabaseUrl, 'function');
  const before = {...process.env};
  try {
    process.env.DATABASE_URL = 'fixture-old-env';
    process.env.PM_SECRET_DIRECTORY = '/runtime-only';
    delete process.env.PM_MIGRATION_SECRET_DIRECTORY;
    assert.throws(() => getMigrationDatabaseUrl(), /MIGRATION_SECRET_REQUIRED/);
    process.env.PM_MIGRATION_SECRET_DIRECTORY = '/missing-migration';
    assert.throws(() => getMigrationDatabaseUrl(), /RUNTIME_SECRET_UNAVAILABLE/);
    delete process.env.PM_SECRET_DIRECTORY; delete process.env.PM_MIGRATION_SECRET_DIRECTORY;
    assert.equal(getMigrationDatabaseUrl(), 'fixture-old-env');
  } finally {
    for (const key of ['DATABASE_URL','PM_SECRET_DIRECTORY','PM_MIGRATION_SECRET_DIRECTORY']) {
