...
const personalSlugs = ['personal-ilchul','personal-flight-app','personal-project-management'];
before(async () => {
  await prisma.flowProject.createMany({data:personalSlugs.map(slug => ({slug,title:slug,scope:'PERSONAL',personalGroup:'PORTFOLIO',position:9999}))});
});
beforeEach(() => prisma.appSetting.deleteMany({ where: { key: { in: keys } } }));
after(async () => {
  await prisma.appSetting.deleteMany({ where: { key: { in: keys } } });
  await prisma.flowProject.deleteMany({where:{slug:{in:personalSlugs}}});
...
test("개인 프로젝트 분류는 조회로 생성하지 않고 변경한 프로젝트만 병합하여 복원한다", async () => {
  ...
  assert.deepEqual(await api.loadUiPreferences(scope), { exists: true, values: Object.fromEntries(personalSlugs.map(slug => [slug,'portfolio'])) });
