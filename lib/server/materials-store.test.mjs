...
test('OCI 참조 변조를 거부하고 객체 삭제 실패는 DB 정리 작업으로 남긴다', async () => {
  ...
  await prisma.flowDocument.update({ where: { projectSlug_slug: { projectSlug: projects[0], slug: material.slug } }, data: { document: { ...row.document, content: { ...content, storage } } } });
  ...
  assert.deepEqual(own[0].value.storage, storage);
  await prisma.appSetting.delete({ where: { key: own[0].key } });
});
