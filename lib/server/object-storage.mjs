...
export function validateReference(ref, project, slug) {
  ...
  if (!config || !ref || ref.provider !== 'oci' || ref.version !== 1 ||
      ...
      !/^[a-f0-9]{64}$/.test(ref.sha256) || !Number.isSafeInteger(ref.bytes) || ref.bytes < 1 || ref.bytes > MAX_OBJECT_BYTES ||
      ref.key !== `materials/${encodeURIComponent(project)}/${encodeURIComponent(slug)}/${ref.sha256}`) throw new Error('자료 저장소 참조가 올바르지 않습니다.');
  return ref;
}
export async function readObject(ref, project, slug) {
  validateReference(ref, project, slug);
...
export async function putObjectWithStatus(bytes, project, slug, contentType) {
  const config = storageConfig();
  if (!config || !bytes.length || bytes.length > MAX_OBJECT_BYTES) throw new Error('OCI 자료 저장 설정 또는 크기가 올바르지 않습니다.');
  const hash = sha256(bytes);
  const ref = { provider: 'oci', version: 1, ...config, key: `materials/${encodeURIComponent(project)}/${encodeURIComponent(slug)}/${hash}`, sha256: hash, bytes: bytes.length };
  const c = await client();
  let created = true;
  try {
  ...
  try { await readObject(ref, project, slug); }
  catch (error) {
    if (created) {
      try { await deleteObject(ref, project, slug); }
...
export async function putObject(bytes, project, slug, contentType) {
  return (await putObjectWithStatus(bytes, project, slug, contentType)).ref;
}
export async function deleteObject(ref, project, slug) {
  validateReference(ref, project, slug);
  try {
