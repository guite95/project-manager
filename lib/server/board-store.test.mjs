...
test("선택한 여러 이슈를 한 번에 오늘로 옮기고 지정 순서를 유지한다", async () => {
  await addIssue("a");
  await addIssue("b");
  await addIssue("c");
  await store.moveIssue("c", "today", "2026-09-09");
  assert.equal(await store.movePoolIssues({ ids: ["b", "a"], action: "today", today: "2026-09-09" }), 2);
  const result = await store.loadBoard("2026-09-09");
  assert.deepEqual(result.today.map((item) => item.id), ["c", "b", "a"]);
  assert.deepEqual(result.issues, []);
  assert.ok(result.today.every((item) => item.done === false));
});

test("프로젝트로 묶음 이동할 때 일부가 없거나 오늘 항목이면 전부 보류한다", async () => {
  await store.createCustomProject({ slug: "target", title: "대상", now: NOW });
  await addIssue("a");
  await addIssue("b");
  await addIssue("c");
  const before = await prisma.issue.findMany({ orderBy: { id: "asc" } });
  await assert.rejects(
    () => store.movePoolIssues({ ids: ["a", "missing"], action: "project", projectSlug: "target" }),
    (error) => error.status === 409,
  );
  await assert.rejects(
    () => store.movePoolIssues({ ids: ["a", "a"], action: "project", projectSlug: "target" }),
    (error) => error.status === 400,
  );
  assert.deepEqual(await prisma.issue.findMany({ orderBy: { id: "asc" } }), before);
  assert.equal(await store.movePoolIssues({ ids: ["a", "c"], action: "project", projectSlug: "target" }), 2);
  const moved = await prisma.issue.findMany({ orderBy: { id: "asc" } });
  assert.deepEqual(moved.map((item) => item.projectSlug), ["target", "tns", "target"]);
  assert.deepEqual(moved.map((item) => item.position), before.map((item) => item.position));
  await assert.rejects(
    () => store.movePoolIssues({ ids: ["a"], action: "project", projectSlug: "unknown" }),
    (error) => error.status === 409,
  );
  assert.deepEqual(await prisma.issue.findMany({ orderBy: { id: "asc" } }), moved);
  await store.moveIssue("b", "today", "2026-09-09");
  await assert.rejects(
    () => store.movePoolIssues({ ids: ["a", "b"], action: "project", projectSlug: "target" }),
    (error) => error.status === 409,
  );
  assert.equal((await prisma.issue.findUnique({ where: { id: "a" } })).projectSlug, "target");
});

test("숨긴 프로젝트로는 옮기지 않고 개인 공용 목록은 대상으로 허용한다", async () => {
  const slug = `batch-hidden-${Date.now()}`;
  await prisma.flowProject.create({ data: { slug, title: "숨김", position: 9999, showInTasks: false } });
  try {
    await addIssue("a");
    await assert.rejects(
      () => store.movePoolIssues({ ids: ["a"], action: "project", projectSlug: slug }),
      (error) => error.status === 409,
    );
    assert.equal((await prisma.issue.findUnique({ where: { id: "a" } })).projectSlug, "tns");
    assert.equal(await store.movePoolIssues({ ids: ["a"], action: "project", projectSlug: "__personal_issues__" }), 1);
  } finally {
    await prisma.flowProject.delete({ where: { slug } });
  }
});
