...
test('개인 프로젝트 등록은 백업 후 이름만 변경하고 신규 등록·재실행 시 기존 데이터를 보존한다', async () => {
  ...
  try {
    ...
    await prisma.flowProject.createMany({ data: [
      { slug: 'personal-flight-app', title: 'flight-app', scope:'PERSONAL', personalGroup:'PORTFOLIO', intro: 'preserve flight intro', position: 100 },
    ...
    assert.deepEqual(await prisma.flowProject.findUnique({ where: { slug: 'personal-flight-app' } }), {
      slug: 'personal-flight-app', title: '해봉티켓', scope:'PERSONAL',personalGroup:'PORTFOLIO',revision:0,intro: 'preserve flight intro', position: 100,
    });
