...
const testSlugs = ["test-sidebar-company", "personal-flight-app", "personal-galaxytty"];
before(async () => {
  await prisma.flowProject.createMany({ data: testSlugs.map(slug => ({ slug, title: slug, position: 9999 })) });
});
beforeEach(() => prisma.appSetting.deleteMany({where:{key:KEY}}));
after(async () => {
  await prisma.appSetting.deleteMany({where:{key:KEY}});
  await prisma.flowProject.deleteMany({ where: { slug: { in: testSlugs } } });
  await prisma.$disconnect();
});

test("저장 전에는 기본 순서, 저장 후에는 개인 프로젝트를 포함한 순서를 복원한다", async () => {
  ...
  for (const slug of testSlugs) assert.ok(initial.includes(slug));
