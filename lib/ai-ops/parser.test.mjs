...
test("NUL in conversation is made visible before database transport", () => {
  const r = parseEntry(entry("response_item", {
    type: "message", role: "user", id: "nul", content: "before\u0000after",
  }), state(), context);
  assert.equal(r.message.body, "before\\u0000after");
  assert.equal(r.session.title, "before\\u0000after");
});

test('source descriptors survive collection while wrapped tool and skill payloads do not upload',()=>{
 const make=(body,role='user')=>entry('response_item',{type:'message',role,channel:role==='assistant'?'final':undefined,id:'stable',content:[{type:'text',text:body}]});
 const mixed=parseEntry(make('<skill>\nprivate instruction\n</skill>\n실제 질문'),state(),context);
 assert.equal(mixed.message.body,'실제 질문');
 assert.equal(mixed.message.sourceMetadata.eventType,'response_item');
 assert.deepEqual(mixed.message.sourceMetadata.contentTypes,['text']);
 assert.deepEqual(parseEntry(make('[external_agent_tool_result]\nError: private tool result','assistant'),state(),context),{});
 assert.ok(parseEntry(make('```xml\n<skill>\nexample\n</skill>\n```','assistant'),state(),context).message.body.includes('<skill>'));
});
