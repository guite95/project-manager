...
test("local DB: idempotence, ownership, retention, null totals, filters and message search", async (t) => {
  ...
  try {
    ...
    for (const name of ["20260917090000_ai_search", "20260917120000_ai_search_provenance"])
      await c.query(await readFile(new URL(`../../prisma/migrations/${name}/migration.sql`, import.meta.url), "utf8"));
    const db = {
    ...
    await c.query("DELETE FROM ai_ops_message WHERE id IN ('expired-body','expired-year')");
    const detail = await querySession(db, "test-session");
    assert.equal(detail.messages[0].body, "literal 100% prompt");
    assert.equal(detail.messages[0].expired, false);
    assert.equal(
    ...
    assert.equal((await querySession(db, "test-session")).session.title, "expired prompt title");
    assert.equal((await queryOverview(db)).sessions[0].title, "expired prompt title");
    assert.equal(
      (await searchMessages(db, { query: "literal" })).messages[0].title,
      "expired prompt title",
    );
    ...
    const ancient = batch();
    const ancientTime = new Date(Date.now() - 800 * 86400000).toISOString();
    ancient.sessions[0] = { ...ancient.sessions[0], id: 'ancient-session', externalId: 'ancient-external', title: 'Permanent history', startedAt: ancientTime, lastActiveAt: ancientTime };
    ancient.messages[0] = { ...ancient.messages[0], id: 'ancient-message', sessionId: 'ancient-session', occurredAt: ancientTime, body: 'permanent archive needle' };
    ancient.usage[0] = { ...ancient.usage[0], id: 'ancient-usage', sessionId: 'ancient-session', occurredAt: ancientTime };
    await ingestBatch(db, ancient);
    assert.equal((await querySession(db, 'ancient-session')).messages[0].body, 'permanent archive needle');
    assert.equal((await searchMessages(db, { query: 'archive needle' })).messages[0].sessionId, 'ancient-session');
    assert.ok((await queryOverview(db)).sessions.some(s => s.id === 'ancient-session'));
    assert.equal((await c.query("SELECT count(*)::int AS n FROM ai_ops_usage WHERE id='ancient-usage'")).rows[0].n, 1);
    await c.query("UPDATE ai_ops_message SET body=NULL WHERE id='ancient-message'");
    await ingestBatch(db, ancient);
    assert.equal((await querySession(db, 'ancient-session')).messages[0].body, 'permanent archive needle');
    ancient.messages[0].body = 'must not overwrite';
    await ingestBatch(db, ancient);
    assert.equal((await querySession(db, 'ancient-session')).messages[0].body, 'permanent archive needle');
  } finally {
