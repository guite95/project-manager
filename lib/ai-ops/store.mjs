...
export async function searchMessages(pool, p = {}) {
  ...
  f.values.push(limit + 1);
  const r = await pool.query(
    `SELECT ${messageSelect},s.title AS title,s.cwd,s.source,d.name AS "deviceName" FROM ai_ops_message m JOIN ai_ops_session s ON s.id=m.session_id JOIN ai_ops_device d ON d.id=s.device_id WHERE ${f.where.join(" AND ")} ORDER BY m.occurred_at DESC,m.id DESC LIMIT $${f.values.length}`,
    f.values,
  );
  const pag = paginated(r.rows, limit, "occurredAt");
