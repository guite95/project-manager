...
export function createPool() {
  ...
  if (!connectionString) throw new Error('DATABASE_CONFIGURATION');
  return new pg.Pool({ connectionString, max: 4 });
}
// Next builds import HTTP route modules without runtime secrets. Defer configuration
// until actual use, but never let pg silently choose its default database identity.
export function createLazyPool() {
  let pool;
  return new Proxy(/** @type {import('pg').Pool} */ ({}), {
    get(_target, property) {
      if (property === 'end' && !pool) return async () => {};
      const current = pool ??= createPool();
