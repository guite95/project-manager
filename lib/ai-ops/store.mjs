  ...
  try {
    ...
    for (const m of b.messages) {
      ...
      if (existing.rowCount && existing.rows[0].session_id !== m.sessionId)
        bad();
      const r = await c.query(
        `INSERT INTO ai_ops_message(id,session_id,role,model,occurred_at,body,chars) VALUES($1,$2,$3,$4,$5,$6,$7) ON CONFLICT(id) DO UPDATE SET body=EXCLUDED.body WHERE ai_ops_message.body IS NULL AND ai_ops_message.session_id=EXCLUDED.session_id`,
        [
          m.id,
          m.sessionId,
          m.role,
...
export async function searchMessages(pool, p = {}) {
  ...
  if (cur) {
    if (keyword && (cur.length !== 3 || !Number.isFinite(cur[2]) || cur[2] < 0))
      bad();
    f.values.push(...cur.slice(0, 2));
    f.where.push(
      keyword
        ? `(${rank},m.occurred_at,m.id)<(${Number(cur[2])}::real,$${f.values.length - 1}::timestamptz,$${f.values.length})`
        : `(m.occurred_at,m.id)<($${f.values.length - 1}::timestamptz,$${f.values.length})`,
    );
  }
  f.values.push(limit + 1);
  const r = await pool.query(
    `SELECT ${messageSelect},m.occurred_at::text AS "cursorOccurredAt",${rank} AS rank,s.title AS title,s.cwd,s.source,d.name AS "deviceName" FROM ai_ops_message m JOIN ai_ops_session s ON s.id=m.session_id JOIN ai_ops_device d ON d.id=s.device_id WHERE ${f.where.join(" AND ")} ORDER BY ${keyword ? "rank DESC," : ""}m.occurred_at DESC,m.id DESC LIMIT $${f.values.length}`,
    f.values,
  );
  const pag = paginated(r.rows, limit, "cursorOccurredAt");
