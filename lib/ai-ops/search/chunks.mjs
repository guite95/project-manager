...
import { interpretMessage, classifyConversation, INTERPRETATION_VERSION } from "./interpretation.mjs";
export const CHUNK_VERSION = "conversation-2";
...
export function chunkMessage(message) {
  ...
  for (const block of interpretMessage(message)) {
    ...
    while (start < block.end) {
      ...
      const local = ["CONVERSATION", "CODE_EXPLANATION", "TEMPLATE"].includes(block.contentType)
        ? classifyConversation(content, message.role, block.evidence) : block;
      chunks.push({ ...block, ...local, content, start, end, index: chunks.length });
      start = end;
    }
  }
  return chunks;
