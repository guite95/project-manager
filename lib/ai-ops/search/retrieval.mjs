...
import { selectResults, selectionConfig } from "./ranking.mjs";
import { CHUNK_VERSION, bytePrefix } from "./chunks.mjs";
import { interpretMessage, searchExcerpt } from "./interpretation.mjs";
import { searchableMessageSql } from "../search-policy.mjs";
const messageFields = `m.id,m.session_id AS "sessionId",m.role,m.model,m.occurred_at AS "occurredAt",m.body,m.source_metadata,m.chars,false AS expired,
...
async function lexicalCandidates(pool, p) {
  ...
  f.values.push(CHUNK_VERSION);
  const version = `$${f.values.length}`;
  f.where.push(`NOT EXISTS(SELECT 1 FROM ai_ops_search_document indexed WHERE indexed.message_id=m.id AND indexed.kind='CHUNK' AND indexed.chunk_version=${version} AND indexed.source_revision=s.search_revision)`);
  ...
  return rows.flatMap(r => {
    const block = searchExcerpt(r, p.query);
    if (!block) return [];
    const lower = block.content.toLowerCase();
    const positions = (p.query.toLowerCase().match(/[\p{L}\p{N}_]+/gu) ?? []).map(t => lower.indexOf(t)).filter(n => n >= 0);
    let offset = Math.max(0, (positions.length ? Math.min(...positions) : 0) - 250);
    if (offset > 0 && /[\uDC00-\uDFFF]/.test(block.content[offset])) offset--;
    const snippet = bytePrefix(block.content.slice(offset), 6000), start = block.start + offset;
    return [{ ...r, importance: block.importance, snippet, metadata: { start, end: start + snippet.length } }];
  });
}
async function chunkLexicalCandidates(pool, p, sessionIds) {
  ...
  const { rows } = await pool.query(
    `SELECT ${messageFields},d.id AS "documentId",d.content AS snippet,d.metadata,d.importance,d.content_type AS "contentType",
...
async function vectorCandidates(pool, p, config, vector, summary, sessionIds) {
  ...
  const { rows } = await pool.query(
    `SELECT ${summary ? 's.id AS "sessionId",d.id' : `${messageFields},d.id AS "documentId",d.content AS snippet,d.metadata,d.importance,d.content_type AS "contentType"`},${distance} AS distance
...
async function linkedQuestions(pool, p, messages) {
  const ids = [...new Set(messages.flatMap(m => (m.metadata?.contextSources ?? []).map(s => s.messageId)))];
  if (!ids.length) return new Map();
  const f = messageFilters(p);
  f.values.push(ids);
  const { rows } = await pool.query(`SELECT m.id,m.body,m.role,m.source_metadata,m.occurred_at AS "occurredAt",m.session_id AS "sessionId"
    FROM ai_ops_message m JOIN ai_ops_session s ON s.id=m.session_id WHERE ${f.where.join(" AND ")}
    AND m.id=ANY($${f.values.length}::text[])`, f.values);
...
export async function retrieve(pool, p = {}, options = {}) {
  ...
  if (!embeddingsEnabled()) {
    const result = await searchMessages(pool, { ...p, mode: "keyword" });
    return { ...result, mode: "keyword", fallbackReason: "EMBEDDING_PAUSED" };
  }
  const limit = page(p),
    config = options.config ?? options.provider?.config ?? embeddingConfig();
  const lexical = await lexicalCandidates(pool, p);
  const chunkLexical = await chunkLexicalCandidates(pool, p);
  const {
    rows: [cap],
  } = await pool.query(
    "SELECT to_regclass('ai_ops_embedding_vector') IS NOT NULL AS ready",
  );
  let vectorRows = [], scopedRows = [],
    candidateSessions = [],
    fallbackReason = null;
  ...
  const selection = options.selection ?? selectionConfig(config);
  const messages = selectResults([vectorRows, scopedRows, chunkLexical, lexical], p.query, { ...selection, limit });
  const questions = await linkedQuestions(pool, p, messages);
  let remaining = 16000;
  const context = [], cited = new Set();
  const cite = (sessionId, messageId, documentId, content, start) => {
    const key = `${messageId}:${start}`;
    if (remaining <= 0 || cited.has(key)) return;
    const excerpt = bytePrefix(content, Math.min(4000, remaining));
    remaining -= Buffer.byteLength(excerpt); cited.add(key);
    context.push({ sessionId, messageId, documentId, content: excerpt, start, end: start + excerpt.length });
  };
  for (const message of messages) {
    cite(message.sessionId, message.id, message.documentId ?? null,
      message.snippet ?? message.body ?? "", message.metadata?.start ?? 0);
    const source = message.metadata?.contextSources?.[0];
    const question = source && questions.get(source.messageId);
    if (!question || question.sessionId !== message.sessionId) continue;
    const block = interpretMessage(question).find(b => b.searchable && b.start <= source.start && b.end >= source.end);
    if (!block) continue;
    const content = question.body.slice(source.start, source.end);
    message.question = { messageId: question.id, content, start: source.start, end: source.end, occurredAt: question.occurredAt };
    cite(question.sessionId, question.id, null, content, source.start);
  }
  return {
    messages,
    nextCursor: null,
