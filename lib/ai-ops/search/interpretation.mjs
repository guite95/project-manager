...
export const INTERPRETATION_VERSION = "content-2";
...
export function classifyConversation(content, role, origin = "legacy-role") {
  ...
  if (lines.length >= 8 && mechanical / lines.length > 0.6)
    return lines.some(l => errorLine.test(l))
      ? policy("ERROR_CONTEXT", "mechanical-output-with-error", "structured-error-line")
      : policy("MACHINE_OUTPUT", "mechanical-line-majority", "heuristic");
...
export function interpretMessage(message) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    add(cursor, start, classifyConversation(body.slice(cursor, start), message.role, origin));
    add(start, end, policy("INSTRUCTION", `envelope:${tag[1]}`, "provider-marker"));
    cursor = end;
    while (i + 1 < lines.length && lines[i + 1].index < end) i++;
  }
  add(cursor, body.length, classifyConversation(body.slice(cursor), message.role, origin));
  return blocks;
}
