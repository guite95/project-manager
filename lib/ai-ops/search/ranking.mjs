import { interpretMessage, lexicalEvidence } from "./interpretation.mjs";
...
export function selectResults(lists, query, { limit = 50, minCosine = 0.68 } = {}) {
  const valid = lists.map(rows => rows.flatMap(row => {
    ...
    if (!block) return [];
    return [{ ...row, importance: Math.min(row.importance ?? 1, block.importance),
      contentType: block.contentType, classificationReason: block.reason }];
  }));
