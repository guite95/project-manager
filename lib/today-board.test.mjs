...
test("완료한 항목만 프로젝트별로 묶어 작업내용 텍스트를 만든다", () => {
  const start = makeBoard({
    ...
    today: [
      ...
      { ...issue("e", "common", "공통도 안 끝냄"), done: false },
    ],
    projectOrder: ["custom-1", "tns", "common"],
  });

  assert.equal(
    board.formatWorklog(start, REGISTRY),
    [
      "09/08 작업내용",
      "`포커스`",
      "- 피드백 내용 1차 완료 및 배포",
...
test("완료한 항목이 없으면 빈 문자열을 준다", () => {
  ...
  assert.equal(board.formatWorklog(start, REGISTRY), "");
  assert.equal(board.formatWorklog(makeBoard(), REGISTRY), "");
});
...
test("롤오버해도 접힘 상태는 그대로 남는다", () => {
  ...
  assert.deepEqual(after.collapsedProjects, ["tns"]);
});
