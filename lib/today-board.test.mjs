...
test("완료한 항목만 프로젝트별로 묶어 작업내용 텍스트를 만든다", () => {
  const start = makeBoard({
    customProjects: [customProject("custom-1", "포커스")],
    today: [
      { ...issue("a", "custom-1", "피드백 내용 1차 완료 및 배포"), done: true },
      { ...issue("b", "custom-1", "피드백 내용 2차 완료 및 배포 예정"), done: true },
      { ...issue("c", "tns", "재고관리 홀딩 버튼 견적에 연결"), done: true },
      { ...issue("d", "tns", "아직 안 끝낸 일"), done: false },
      { ...issue("e", "common", "완료한 공통 업무"), done: true },
    ],
    projectOrder: ["custom-1", "tns", "common"],
  });

  assert.equal(
    board.formatWorklog(start, REGISTRY, "2026-09-08"),
    [
      "09/08 작업내용",
      "`포커스`",
      "- 피드백 내용 1차 완료 및 배포",
      "- 피드백 내용 2차 완료 및 배포 예정",
      "`티앤에스`",
      "- 재고관리 홀딩 버튼 견적에 연결",
    ].join("\n"),
  );
...
test("공통만 완료했으면 복사할 내용이 없고 원본 항목은 유지한다", () => {
  const start = makeBoard({
    today: [
      { ...issue("a", "common", "완료한 공통 업무"), done: true },
      { ...issue("b", "tns", "진행 중인 프로젝트 업무"), done: false },
    ],
  });
  const original = structuredClone(start);

  assert.equal(board.formatWorklog(start, REGISTRY, "2026-09-14"), "");
  assert.deepEqual(start, original);
});

test("공통 프로젝트가 목록에 없어도 공통 항목은 미분류로 복사하지 않는다", () => {
  const start = makeBoard({
    today: [
      { ...issue("a", "common", "완료한 공통 업무"), done: true },
      { ...issue("b", "tns", "완료한 프로젝트 업무"), done: true },
    ],
  });

  assert.equal(
    board.formatWorklog(start, REGISTRY.filter((project) => project.slug !== "common"), "2026-09-14"),
    ["09/14 작업내용", "`티앤에스`", "- 완료한 프로젝트 업무"].join("\n"),
  );
});

test("완료한 항목이 없으면 빈 문자열을 준다", () => {
  const start = makeBoard({
    today: [{ ...issue("a", "tns"), done: false }],
  });

  assert.equal(board.formatWorklog(start, REGISTRY, "2026-09-09"), "");
  assert.equal(board.formatWorklog(makeBoard(), REGISTRY, "2026-09-09"), "");
});

test("소속 프로젝트가 사라진 완료 항목은 미분류로 묶는다", () => {
  const start = makeBoard({
    today: [{ ...issue("a", "지워진프로젝트", "고아가 된 일"), done: true }],
  });

  assert.equal(
    board.formatWorklog(start, REGISTRY, "2026-12-25"),
    ["12/25 작업내용", "`미분류`", "- 고아가 된 일"].join("\n"),
  );
});
