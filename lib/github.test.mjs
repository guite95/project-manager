...
import { parseGitHubRepositoryUrl } from './github.ts';
import { verifyGitHubRepository, exchangeGitHubCode } from './server/github-api.ts';
...
test('OAuth 교환은 PKCE를 보내고 만료 없는 토큰을 거부한다',async()=>{
  const config={clientId:'client',callbackUrl:'https://pm.example/api/github/callback'};
  await assert.rejects(exchangeGitHubCode(config,'secret','code','verifier',async(url,init)=>{
    ...
    return ok({access_token:'ghu_test',token_type:'bearer'});
  }),/만료/);
});
