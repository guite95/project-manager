...
test("CLI collects, validates and saves real Git + completion evidence; hash and revision protect writes", async () => {
  ...
  try {
    ...
    assert.equal(JSON.parse(collected.stdout).aiTranscript, undefined);
    await assert.rejects(stat(`${evidence}.ai.json`), { code: "ENOENT" });
    ...
    assert.equal(stored.value.evidence.sources.filter(s => s.kind === "ai").length, 0);
