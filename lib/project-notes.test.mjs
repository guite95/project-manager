...
test("공통 명심할 점을 지원하고 고객 프로젝트에는 표시하지 않는다", () => {
  assert.equal(projectNotes.hasProjectNotes({slug:"common",scope:"COMPANY"}), true);
  assert.equal(projectNotes.hasProjectNotes({slug:"tns",scope:"COMPANY"}), false);
  assert.equal(projectNotes.hasProjectNotes(undefined), false);
