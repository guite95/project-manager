...
export function buildPmSecretEntries(input, existing) {
  try {
    ...
    if (!['postgresql:', 'postgres:'].includes(url.protocol) || decodeURIComponent(url.pathname) !== '/project_management'
      || ['host', 'hostaddr', 'port'].some(key => url.searchParams.has(key))) throw new Error();
    const retained = input.application_signing_secrets.project_management_legacy_session.new_secret;
    if (!retained || retained !== existing.SESSION_SECRET || typeof existing.APP_PASSWORD_HASH !== 'string' || !existing.APP_PASSWORD_HASH) throw new Error();
    const entries = ['runtime', 'migration'].map(kind => {
      const account = input.service_accounts[`project_management_postgres_${kind}`];
      if (!account?.new_username || !account.new_password) throw new Error();
      const updated = new URL(url);
      updated.username = encodeURIComponent(account.new_username); updated.password = encodeURIComponent(account.new_password);
      return { name: `pm-${kind}-database-url`, value: updated.toString() };
    });
    return [...entries, { name: 'pm-legacy-session-secret', value: retained }, { name: 'pm-bootstrap-password-hash', value: existing.APP_PASSWORD_HASH }];
  } catch { throw new Error('PM_SECRET_REGISTRATION_INPUT_INVALID'); }
}
...
try:
 ...
 print(json.dumps({'ok':False,'code':'PM_VAULT_REGISTRATION_FAILED','partial_creations_may_exist':True}))
 sys.exit(1)
`;
...
async function main() {
  ...
  const remote = String.raw`docker exec project-management node -e 'const names=["DATABASE_URL","SESSION_SECRET","APP_PASSWORD_HASH"]; if(Object.hasOwn(process.env,"PM_SECRET_DIRECTORY"))process.exit(1); process.stdout.write(JSON.stringify(Object.fromEntries(names.map(k=>[k,process.env[k]]))));'`;
  ...
  const entries = buildPmSecretEntries(credentials, existing);
