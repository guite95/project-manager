...
| `/personal` | 개인 공간 기본 화면 — 기록 관리 기능은 추후 추가 |
...
나뉘며, 포트폴리오는 개인 하위 메뉴에, 작성 가이드는 하단 보조 메뉴에 배치한다. 개인과 포트폴리오는
현재 기본 진입 화면만 제공한다. 상세 메뉴를 접어도 주 메뉴는 남으며, 모바일에서는
...
각 프로젝트의 **자료** 메뉴(`/flows/[project]/materials`)에서 PDF·UTF-8 HTML 파일을
제목과 함께 추가한다. 파일당 최대 10MB이며, 파일 형식·내용·크기를 서버에서도 검사한다.
기존 HTML·발표 콘텐츠도 같은 목록에서 조회한다. 업로드 원본은 `content.kind: "material"`의
base64 데이터와 파일명·크기로 공유 DB에 저장하고, 목록·메뉴에는 파일 본문을 전달하지 않는다.
...
PDF는 브라우저 PDF 뷰어로 표시하고 원본 다운로드를 제공한다. HTML은 스크립트와
