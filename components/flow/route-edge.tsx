...
export function RouteEdge(props: EdgeProps<Edge<RouteData>>) {
  ...
  const [dragPoints, setDragPoints] = useState<FlowPoint[] | null>(null);
  const movedPoint = useRef<{ index: number; point: FlowPoint } | null>(null);
  const points = dragPoints ?? props.data?.waypoints ?? [];
  useEffect(() => { setDragPoints(null); movedPoint.current = null; }, [props.data?.waypoints]);
  ...
  return <>
    ...
    {props.selected && props.data?.movePoint && points.map((p, index) => <circle key={index} cx={p.x} cy={p.y} r={7}
      ...
      onPointerMove={event => {
        ...
        movedPoint.current = { index, point };
        setDragPoints(current => (current ?? points).map((p, i) => i === index ? point : p));
      }}
      onPointerUp={event => {
        event.currentTarget.releasePointerCapture(event.pointerId);
        if (movedPoint.current?.index === index) props.data?.movePoint?.(props.id, index, movedPoint.current.point);
      }}
      onPointerCancel={() => { movedPoint.current = null; setDragPoints(null); }} />)}
  </>;
}
