...
- Company and personal project metadata is DB-owned: `flow_project.scope` (`COMPANY`/`PERSONAL`), `personal_group` (`PORTFOLIO`/`TOY`), title and revision. `project_repository` stores multiple workspace-relative repo paths per project. Menus, access checks, personal issues, notes and Git collection must consume this metadata; never restore a hardcoded personal ID registry. Existing IDs, content and memberships remain stable. See `docs/project-registry.md`.
- `/api/project-registry` requires a server-validated actor: OWNER manages both scopes, ADMIN manages company projects, MEMBER cannot manage metadata. Scope is immutable through normal edits. Preserve explicit personal-project memberships and company admin rights. Revision checks and unique repository bindings must fail without partial updates.
- Personal group preferences read/write `flow_project.personal_group`; the old `ui:personal-project-groups` JSON is retained only as migration history. Sidebar order remains shared in `sidebar:project-order`. GET never creates projects. Initial requested rows may be added with `scripts/register-personal-projects.mjs` after inspection, backup and deployment verification; routine additions use the DB-backed UI without redeployment.
