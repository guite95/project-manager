...
- GitHub repository links are personal-project-only. Verify ownership/collaboration through authenticated `/user/repos` membership, not public repository readability. GitHub App tokens and client secret belong only in OCI Secrets; app_setting stores non-secret links/account data and one-use OAuth request hashes. All GitHub APIs require a valid app session even in development. See `docs/github.md` for initial setup and callback URLs.
