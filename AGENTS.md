...
- GitHub repository links are personal-project-only. Use the owner's fine-grained PATs; no GitHub App installation or callback is required. Verify ownership/collaboration through authenticated `/user/repos` membership, not public repository readability. Tokens belong only in OCI Secrets and are selected per project; app_setting stores non-secret repository links. All GitHub APIs require a valid app session even in development. See `docs/github.md` for token scopes, organization approval limits, and setup.
