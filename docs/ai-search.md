...
현재 임베딩은 중단 상태다. `AI_OPS_EMBEDDING_ENABLED`는 기본 `false`이며 원본 세션·메시지·사용량 수집과 저장은 계속한다. backfill/worker/retry는 DB 변경 없이 `EMBEDDING_PAUSED`를 반환하고, hybrid 검색은 원문 키워드 검색으로 전환한다. 기존 파생 데이터와 벡터는 삭제하지 않는다.

2026-09-21 운영 서버의 `project-management-embedding.timer`를 disable/stop하고 service 중지를 확인했다. 아래 명령과 구조는 재개 후의 운영 참고다. 데이터 전처리 검증 및 사용자의 명시적 재개 요청 전에는 환경변수를 `true`로 설정하거나 타이머를 다시 켜지 않는다. 앱의 검색 경로 차단은 이 코드가 배포된 뒤 적용된다.
...
`conversation-2`로 파생 문서를 재생성하면 기존 알림 벡터의 연결은 제거되고, 변경되지
