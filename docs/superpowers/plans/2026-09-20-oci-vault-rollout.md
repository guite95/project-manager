...
- 2026-09-21 추가 승인: schema 변경 포함 main 자동 배포를 유지한다. 호스트의 고정된 전용 migration 작업은 해당 migration Secret을 읽을 수 있다. 일반 앱에는 runtime Secret만 mount하며 migration 비밀값은 일회성 배포 컨테이너에만 전달한다. Secret 관리/IAM/전체 DB 관리자 권한은 추가하지 않는다.
