...
본문은 90일, 대화 메타데이터/사용량은 365일이다. 조회는 항상 만료 데이터를 숨기고, 수집 작업이 오래된 본문/메타데이터를 정리한다. Agent가 꺼져 있으면 물리 삭제는 다음 성공 수집까지 미뤄지지만 화면에 만료 본문이 나타나지는 않는다. 수집 DB와 별도로 운영 백업의 보관 정책은 관리해야 한다.

모든 날짜 필터는 한국 시간이다. 본문 검색은 최대 최근 90일의 부분 문자열 검색이며 `%`, `_`도 일반 문자로 처리한다. 검색/세션/대화는 커서 페이지네이션이다. UI는 화면이 보일 때 15초마다 갱신한다. 인증 만료/마이그레이션 미적용/서버 오류를 빈 기록과 구분한다.
