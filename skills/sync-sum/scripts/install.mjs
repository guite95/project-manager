...
const targets = ['.agents', '.claude'].map(runtime => resolve(homedir(), runtime, 'skills', 'sync-sum'));
try {
  ...
  for (const target of targets) {
    ...
    else if (!entry.isSymbolicLink() || await realpath(target) !== source) throw new Error(`기존 스킬과 경로가 다릅니다: ${target}`);
  }
  ...
  for (const target of targets) {
    if (await realpath(target) !== source) throw new Error(`설치 경로 확인 실패: ${target}`);
  }
